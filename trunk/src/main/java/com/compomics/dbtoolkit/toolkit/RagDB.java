...
/**
 * This class implements a command-line way of calling the ragging option
 * in the ProcessThread class, albeit it is NOT called in threading mode.
 *
...
public class RagDB {
    ...
    public static void main(String[] args) {
        ...
        if(args == null || args.length == 0) {
            flagError("Usage:\n\tRagDB -(N|C) [--enzyme <enzymeName> [--mc <number_of_missed_cleavages>]] [--filter <filter_name> [--filterParam \"<filter_parameter>\"]] [--truncate <number_of_residues>] [--lowMass <lower_mass_treshold> --highMass <higher_mass_treshold>] --input <input_db_name> <output_db_name>\n\n\tNote that an existing output file will be silently overwritten!");
        }
        ...
        temp = clp.getFlags();
        if((temp == null) || (temp.length == 0)) {
            flagError("You need to specify a terminus to rag (either '-C' OR '-N')!");
        }
        ...
        if(inputFile == null) {
            ...
            flagError("You did not specify a ragging position (-N or -C)!\n\nRun program without parameters for help.");
            ...
            if(terminus.equalsIgnoreCase("N")) {
                term = ProcessThread.NTERMINUS;
            } else if(terminus.equalsIgnoreCase("C")) {
                term = ProcessThread.CTERMINUS;
            } else {
                flagError("You need to specify a terminus to rag, and it can be either 'C' or 'N', but nothing else!");
            }
            ...
            if(!input.exists()) {
                ...
                Properties p = null;
                try {
                    InputStream is = RagDB.class.getClassLoader().getResourceAsStream("DBLoaders.properties");
                    p = new Properties();
                    if(is != null) {
                        p.load(is);
                        is.close();
                ...
                boolean doTrunc = false;
                if(trunc >= 0) {
                    doTrunc = true;
                }
                ProcessThread pt = ProcessThread.getRaggingTask(loader, output, null, f, enzyme, massLimits, minMass, maxMass, term, doTrunc, trunc);
                System.out.println("\nPerforming " + terminus + "-terminal ragging in '" + inputFile + "'.");
                System.out.println("\n\tParameters for this ragging are:");
                ...
                System.out.println("\t\t - Truncation: " + (doTrunc?"truncating to " + trunc + " " + terminus + "-terminal residues.":"no truncation requested."));
