...
/**
 * This class implements a command-line way of calling the sequence-based subset option
...
public class IsolateSubset {
    ...
    public static void main(String[] args) {
        ...
        CommandLineParser clp = new CommandLineParser(args, new String[]{"enzyme", "mc", "filter", "filterParam", "query", "lowMass", "highMass", "input"});
        ...
        if(inputFile == null) {
            ...
            if(!input.exists()) {
                ...
                Properties p = null;
                try {
                    InputStream is = IsolateSubset.class.getClassLoader().getResourceAsStream("DBLoaders.properties");
                    p = new Properties();
                    if(is != null) {
                        p.load(is);
                        is.close();
                ...
                if(enzymeName != null) {
                    try {
                        InputStream in = IsolateSubset.class.getClassLoader().getResourceAsStream("enzymes.txt");
                        if(in != null) {
                            MascotEnzymeReader mer = new MascotEnzymeReader(in);
                            enzyme = mer.getEnzyme(enzymeName);
                            if(enzyme == null) {
                                flagError("The enzyme you specified (" + enzymeName + ") was not found in the Mascot Enzymefile '" + IsolateSubset.class.getClassLoader().getResource("enzymes.txt") + "'!");
                            } else {
                                if(miscl != null) {
                                    try {
                ...
                if(filter != null) {
                    try {
                        ...
                        InputStream in = IsolateSubset.class.getClassLoader().getResourceAsStream("filters.properties");
                        if(in == null) {
                ...
                try {
                    pf = new QueryParser().parseQuery(query);
                } catch(ParseException pe) {
                    flagError("Unable to parse your query!\nParser returned: " + pe.getMessage());
                }
                ...
                ProcessThread pt = ProcessThread.getSubsetTask(loader, output, null, f, enzyme, massLimits, minMass, maxMass, pf);
                System.out.println("\nIsolating subset by query in '" + inputFile + "'.");
